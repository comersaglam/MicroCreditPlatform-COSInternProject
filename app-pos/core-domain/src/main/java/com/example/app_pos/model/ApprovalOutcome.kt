...
sealed interface ApprovalOutcome {
     ...
     *
     * [approvalId] travels back because a caller may have to WAIT for the answer — a sale
     * at the till is holding the payment gateway open and can only close once the customer
     * decides. Callers that simply report "sent" ignore it.
     */
    data class SentForApproval(val approvalId: String) : ApprovalOutcome
    ...
    /**
     * The server could not be reached and nothing was written ANYWHERE.
     *
     * Distinct from [QueuedOffline], which keeps the action on the device: at a till the
     * gate cannot be honoured offline, because the customer cannot be asked and booking
     * the entry regardless is precisely the unilateral write the gate exists to stop. So
     * the sale stops instead, and the merchant is told why.
     */
    data object Unreachable : ApprovalOutcome
