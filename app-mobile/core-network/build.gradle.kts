...
android {
    ...
    buildTypes {
        debug {
            ...
            //
            // Set `mobileApiHost` in gradle.properties rather than editing this line, so a
            // changed LAN address never shows up as a source diff. Named separately from
            // app-pos's `posApiHost` because the two apps are separate Gradle builds and may
            // sit on different devices (one emulator, one physical) during the same test.
            val apiHost = (project.findProperty("mobileApiHost") as String?) ?: "10.0.2.2"
