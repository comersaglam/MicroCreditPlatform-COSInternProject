 ...
 *
 * Both schedules carry the same constraint — a connected network — so a phone with no
 * signal is never woken to fail. The queue simply waits.
 *
 * app-mobile's asymmetry with app-pos is deliberate: this is a battery-powered phone that
 * is not expected to stay awake, so background work here only DRAINS THE QUEUE. It does
 * not poll the server for incoming changes — that runs while the app is in the foreground
 * (Turn 39). A POS sits on a counter and can afford to listen; a phone cannot.
 */
@Singleton
class SyncScheduler @Inject constructor(
     ...
     * period each time. On a phone that is opened many times a day the job would be
     * perpetually rescheduled and never actually run.
     */
    ...
    /**
     * Push now — used right after a write, when the entry is fresh and the user is
     * plausibly online.
     *
     * Handed to WorkManager rather than run in a coroutine because it must survive the app
     * closing: someone who confirms an entry and immediately swipes the app away would
     ...
     *
     * KEEP, so tapping through several writes quickly coalesces into one run instead of
     * queueing a redundant job per write — the drain sends the whole queue anyway.
     */
    fun syncNow() {
