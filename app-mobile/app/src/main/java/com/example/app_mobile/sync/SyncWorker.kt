...
/**
 * Drains the outbox from the background, so writes reach the server even while app-mobile
 * is closed.
 *
 * Until now the queue only emptied at app start and right after a write. That left the case
 * this Worker exists for: a phone that spent the day without signal, was closed, and never
 * reopened before signal returned. WorkManager wakes it instead.
 *
 * Note what it does NOT do: it never READS from the server. On a battery-powered phone the
 * background job is for delivering what the user already did, not for listening — polling
 * for incoming approvals happens in the foreground (Turn 39).
 *
...
class SyncWorker @AssistedInject constructor(
    ...
    companion object {
        /** Shared by the periodic schedule and the one-off after a write (see SyncScheduler). */
        ...
        suspend fun decide(repository: Repository): Result {
            ...
            // ledger entries. Retrying leaves the queue untouched until someone signs in.
